...
function DeleteColumnDialog({ column, columns, onCancel, onConfirm }: {
  ...
  return <div className="decision-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onCancel(); }}>
    <section ref={root} className="decision-dialog" role="alertdialog" aria-modal="true" aria-labelledby="kanban-delete-title" onKeyDown={e => {
      ...
      <h2 id="kanban-delete-title">{t("删除「{0}」？", column.title)}</h2>
      <p>{t("这列有 {0} 张卡片。可以先移动卡片，或确认一并删除。操作可撤销。", column.cards.length)}</p>
      {columns.length > 1 ? <label className="kanban-field">{t("将卡片移动到")}<select aria-label={t("删除列前的目标列")} value={destination} onChange={e => setDestination(e.target.value)}>
          {columns.filter(c => c.from !== column.from).map(c => <option key={c.from} value={c.from}>{c.title || t("未命名列")}</option>)}
        </select>
      </label> : null}
      <div className="decision-actions">
        <button ref={cancel} onClick={onCancel}>{t("取消")}</button>
        {destination !== '' ? <button onClick={() => onConfirm(Number(destination))}>{t("移动卡片并删除列")}</button> : null}
        <button className="destructive" onClick={() => onConfirm()}>{t("删除列及卡片")}</button>
      </div>
    </section>
  </div>;
}
...
export default function KanbanBoard({ board, documentKey, onChange, onHistory }: Props) {
  ...
  function perform(action: KanbanAction, snapshot = board.source) {
    ...
    if (!current.onChange || snapshot !== current.board.source) {
      setNotice(t("文档或操作状态已变化，请重新操作。")); return false;
    }
    const change = kanbanChange(snapshot, action);
    if (!change) { setNotice(t("无法安全执行此操作。请检查名称或卡片 Markdown 格式；原文未修改。")); return false; }
    const next = applySourceChanges(snapshot, change.changes)!;
    expected.current = next;
    if (!current.onChange(change)) {
      expected.current = current.board.source;
      setNotice(t("操作未应用：文档已变化、正在保存切换，或编辑器尚未就绪。")); return false;
    }
    setNotice(t("已更新 Markdown · 可撤销"));
  ...
  useEffect(() => {
    ...
    if (changedDocument || (observed.current.source !== board.source && expected.current !== board.source)) {
      ...
      setNotice(changedDocument ? '' : t("源码已更新，已取消过期的卡片操作。"));
  ...
  const addForm = <div className="kanban-add-form">
    <input ref={field} aria-label={adding === 'column' ? t("新列名称") : t("新卡片标题")} placeholder={adding === 'column' ? t("列名称") : t("写一张卡片…")} value={title} maxLength={1000} disabled={!enabled}
    ...
    <div><small>{t("Enter 添加 · Esc 取消")}</small><button disabled={!enabled || !title.trim()} onClick={add}>{t("添加")}</button><button onClick={() => { setAdding(null); setTitle(''); }}>{t("取消")}</button></div>
  </div>;
  const deletingColumn = deleting?.kind === 'column' && board.columns.find(c => c.from === deleting.from);

  return <div ref={root} className={`kanban-pane${dragging ? ' kanban-dragging' : ''}`} tabIndex={-1} aria-label={t("Markdown 看板")} onKeyDown={e => {
    if (e.nativeEvent.isComposing || e.keyCode === 229) return;
    ...
    <header className="kanban-heading"><div><small>{t("MARKDOWN · 看板")}</small><h1>{board.title}</h1>{board.intro ? <p>{board.intro}</p> : null}</div>
      <div className="kanban-tools"><button disabled={!enabled} onClick={() => { setAdding('column'); setTitle(''); }}>{t("＋ 添加列")}</button><button disabled={!onHistory || !enabled} onClick={() => onHistory?.('undo')}>{t("撤销")}</button><button disabled={!onHistory || !enabled} onClick={() => onHistory?.('redo')}>{t("重做")}</button></div>
    </header>
    <div className="kanban-caption">{t("{0} 列 · {1} 张卡片 · 拖动把手排序，单击卡片编辑。完成勾选与所在列独立。", board.columns.length, board.columns.reduce((sum, c) => sum + c.cards.length, 0))}</div>
    {!enabled ? <p className="kanban-warning" role="status">{t("看板暂不可操作，正在等待当前文档操作完成。")}</p> : null}
    <div ref={scroll} className="kanban-scroll"><div className="kanban-columns">
      {board.columns.map((col, i) => <section key={col.from} className={`kanban-column${feedback?.drop?.before === col.from && drag.current?.kind === 'column' ? ' kanban-column-drop' : ''}`} data-kanban-column={col.from} aria-label={t("列：{0}", col.title || t("未命名列"))}>
        <header className="kanban-column-head"><button className="kanban-grip" aria-label={t("拖动列 {0}", col.title)} disabled={!enabled} onPointerDown={e => startDrag(e, 'column', col.from)}>⠿</button><h2>{col.title || t("未命名列")}</h2><span>{col.cards.length}</span>
          <div className="kanban-column-menu"><button aria-label={t("{0} 列操作", col.title)} aria-expanded={menu === col.from} disabled={!enabled} onClick={() => setMenu(menu === col.from ? null : col.from)}>•••</button>
            {menu === col.from ? <div className="kanban-menu" role="group" aria-label={t("列操作")}>
              <label>{t("列名")}<input aria-label={t("列名")} defaultValue={col.title} maxLength={1000} onChange={e => { if (e.target.value.trim() !== col.title) perform({ type: 'rename-column', column: col.from, title: e.target.value }); }} onKeyDown={e => { if (e.key === 'Enter' && !e.nativeEvent.isComposing) setMenu(null); }} /></label>
              <button disabled={i === 0} onClick={() => { perform({ type: 'move-column', column: col.from, before: board.columns[i - 1].from }); setMenu(null); }}>{t("向左移动")}</button>
              <button disabled={i === board.columns.length - 1} onClick={() => { perform({ type: 'move-column', column: col.from, before: board.columns[i + 2]?.from ?? null }); setMenu(null); }}>{t("向右移动")}</button>
              <button className="destructive" onClick={() => { setDeleting({ kind: 'column', from: col.from, title: col.title, source: board.source }); setMenu(null); }}>{t("删除列…")}</button>
            </div> : null}
          </div>
        </header>
        <div className="kanban-cards">
          {col.cards.map(item => <div key={item.from}>
            ...
            <article className={`kanban-card${item.checked ? ' is-done' : ''}${selected === item.from ? ' is-selected' : ''}`} data-kanban-card={item.from} onClick={e => { if (!(e.target instanceof Element) || !e.target.closest('button,input,label,a')) setSelected(item.from); }}>
              <div className="kanban-card-top"><input type="checkbox" aria-label={t("完成：{0}", item.title || t("无标题卡片"))} checked={item.checked} disabled={!enabled} onChange={e => perform({ type: 'toggle', statusOffset: item.statusOffset, checked: e.target.checked })} />
                <button className="kanban-card-title" onClick={() => { setSelected(item.from); setMenu(null); }}>{item.title || t("无标题卡片")}</button>
                <button className="kanban-grip" aria-label={t("拖动卡片 {0}", item.title)} disabled={!enabled} onPointerDown={e => startDrag(e, 'card', item.from)}>⠿</button>
              </div>
              {item.body ? <p>{item.body}</p> : null}
              <footer>{(item.title.match(/(?:^|\s)#[\p{L}\p{N}_-]+/gu) ?? []).map((tag, n) => <small key={n} className="kanban-tag">{tag.trim()}</small>)}{item.tasks.length ? <small>{item.tasks.filter(t => t.checked).length}/{item.tasks.length} {t("子任务")}</small> : null}</footer>
            </article>
          </div>)}
          {drag.current?.kind === 'card' && feedback?.drop?.column === col.from && feedback.drop.before === null ? <div className="kanban-drop-marker" /> : null}
          {!col.cards.length ? <div className="kanban-empty-column">{t("暂无卡片 · 可拖入这里")}</div> : null}
        </div>
        {adding === col.from ? addForm : <button className="kanban-add-card" disabled={!enabled} onClick={() => { setAdding(col.from); setTitle(''); }}>{t("＋ 添加卡片")}</button>}
      </section>)}
      {adding === 'column' ? <div className="kanban-new-column">{addForm}</div> : <button className="kanban-add-column" disabled={!enabled} onClick={() => { setAdding('column'); setTitle(''); }}>{t("＋ 添加列")}</button>}
    </div></div>
    <div className="kanban-notice" role="status" aria-live="polite">{notice || t("文件仍是 Markdown · 修改自动保存到原文件")}</div>
    {card && cardColumn ? <aside className="kanban-detail" aria-label={t("卡片详情")}>
      <header><span>{t("卡片详情")}</span><button aria-label={t("关闭卡片详情")} onClick={closeDetails}>×</button></header>
      <div className="kanban-detail-body">
        <label className="kanban-field">{t("标题")}<input ref={detailTitle} aria-label={t("卡片标题")} maxLength={1000} value={card.title} disabled={!enabled} onChange={e => perform({ type: 'edit-card', card: card.from, title: e.target.value })} /></label>
        <label className="kanban-check"><input type="checkbox" aria-label={t("卡片已完成")} checked={card.checked} disabled={!enabled} onChange={e => perform({ type: 'toggle', statusOffset: card.statusOffset, checked: e.target.checked })} />{t("已完成（不自动移动列）")}</label>
        <label className="kanban-field">{t("所在列")}<select aria-label={t("卡片所在列")} value={cardColumn.from} disabled={!enabled} onChange={e => perform({ type: 'move-card', card: card.from, column: Number(e.target.value), before: null })}>{board.columns.map(c => <option key={c.from} value={c.from}>{c.title || t("未命名列")}</option>)}</select></label>
        <div className="kanban-tools"><button disabled={!enabled || cardColumn.cards[0] === card} onClick={() => perform({ type: 'move-card', card: card.from, column: cardColumn.from, before: cardColumn.cards[cardColumn.cards.indexOf(card) - 1].from })}>{t("上移卡片")}</button><button disabled={!enabled || cardColumn.cards[cardColumn.cards.length - 1] === card} onClick={() => perform({ type: 'move-card', card: card.from, column: cardColumn.from, before: cardColumn.cards[cardColumn.cards.indexOf(card) + 2]?.from ?? null })}>{t("下移卡片")}</button></div>
        <label className="kanban-field">{t("说明与子任务")}<textarea aria-label={t("卡片说明与子任务")} value={bodyDraft?.from === card.from && bodyDraft.source === board.source ? bodyDraft.value : card.body.replace(/\r\n?/g, '\n')} disabled={!enabled} onChange={e => {
        ...
        <p className="kanban-help">{t("使用 Markdown 编辑说明。输入「- [ ] 子任务」添加子任务；写回文件时自动缩进。没有独立保存按钮。")}</p>
      ...
      <footer><small>{t("修改进入同一份 Markdown 和撤销历史")}</small><button className="destructive" disabled={!enabled} onClick={() => setDeleting({ kind: 'card', from: card.from, title: card.title, source: board.source })}>{t("删除卡片…")}</button></footer>
    </aside> : null}
    {deletingColumn && deletingColumn.cards.length ? <DeleteColumnDialog column={deletingColumn} columns={board.columns} onCancel={() => setDeleting(null)} onConfirm={confirmDelete} />
      : deleting ? <DecisionDialog title={t("删除{0}？", deleting.kind === 'card' ? t("卡片") : t("空列"))} message={t("「{0}」将从 Markdown 中移除，可使用撤销恢复。", deleting.title)} confirmLabel={t("删除")} cancelLabel={t("取消")} destructive onCancel={() => setDeleting(null)} onConfirm={() => confirmDelete()} /> : null}
    {feedback && drag.current?.moved ? <div className="kanban-drag-ghost" style={{ left: feedback.x + 12, top: feedback.y + 12 }}>{drag.current.kind === 'card' ? t("移动卡片") : t("移动列")}</div> : null}
  </div>;
}
