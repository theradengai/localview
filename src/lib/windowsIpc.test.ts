...
function create(href = 'http://tauri.localhost/', top = true, development = false) {
  ...
  const bridge = { runCallback: callback, convertFileSrc: (command: string, protocol: string) => `http://${protocol}.localhost/${command}` };
  const window: Record<string, unknown> = { location: { href }, __TAURI_INTERNALS__: bridge, ipc: { postMessage: native } };
  ...
  return { post: (bridge as typeof bridge & { postMessage: (value: unknown) => void }).postMessage, callback, native, fetcher };
}
