...
export function validatePastedImages(files: File[]): void {
  if (!files.length || files.length > MAX_PASTED_IMAGES) throw new Error(t("一次最多粘贴 8 张图片"));
  if (files.some((file) => !IMAGE_TYPES.has(file.type))) throw new Error(t("暂时支持 PNG、JPEG、GIF 和 WebP 图片"));
  if (files.some((file) => file.size === 0)) throw new Error(t("剪贴板图片为空"));
  if (files.reduce((total, file) => total + file.size, 0) > MAX_PASTED_IMAGE_BYTES) {
    throw new Error(t("粘贴图片总大小不能超过 10 MB"));
  }
}
...
export function readPastedImage(file: File): Promise<PastedImage> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error(t("无法读取剪贴板图片")));
    reader.onabort = () => reject(new Error(t("读取剪贴板图片已取消")));
    reader.onload = () => {
      if (!(reader.result instanceof ArrayBuffer)) return reject(new Error(t("无法读取剪贴板图片")));
