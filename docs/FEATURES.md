...
- Creation actions support empty Markdown files and ordinary folders; other-file creation and copy are not included. Ordinary files, ordinary subfolders, and whole `.numbers`/`.pages`/`.key` document bundles can be renamed inline, but the workspace root, symlinks, bundle contents, extensions, case-only names, collisions, and batch rename are not supported. The current editable file, or a folder containing it, must save successfully before rename; unrelated dirty documents are not flushed. Confirmed renames migrate the selected path, save target, loaded descendants, session, and path-keyed renderer, while an uncertain filesystem outcome only refreshes the affected directory and is never reported as success. Ordinary files, non-empty ordinary folders, and whole iWork bundles may also move atomically inside the same workspace; bundle internals cannot be moved independently or used as destinations. macOS resolves the final exclusive no-follow operation from the pinned workspace root, verifies the resulting identities, and refreshes rather than claiming success when the outcome cannot be proved. LocalView rejects the workspace root as a source, symlinks, outside-workspace paths, same-parent moves, folder self/descendant targets, destination collisions, changed identities, and cross-volume moves. Files and workspace subfolders can also move to Trash, but never be permanently deleted in LocalView.
- Markdown Preview and Split task checkboxes update the original source through auto-save and preserve conflict protection. Each preview toggle supports undo/redo. Task continuations with deep indentation are repaired for display only; existing code and links remain protected.
