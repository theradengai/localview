...
- Desktop windows synchronize through revisioned snapshots ordered by the native process. Rapid selections are serialized; stale native notifications and unversioned storage echoes cannot roll back the latest accepted choice. New desktop windows hydrate from that process state, while a complete relaunch seeds it from the saved preference. The browser demo retains ordinary storage-event synchronization.
...
Locale tests cover system detection, persistence failures, browser storage propagation, ordered native selections, stale acknowledgments, listener cleanup, placeholders, opaque user data, the real CodeMirror instance/selection/history, application drafts and spreadsheet paging without a second file read. The isolated Chromium/WebKit checks cover both languages, preference persistence/propagation and board source preservation. They use only synthetic browser-demo documents, not an installed LocalView or private local files.
