...
Download from the [official release page](https://github.com/theradengai/localview/releases/tag/v0.2.0-beta.6). Quit the existing LocalView normally so pending edits can save. Open the DMG and drag LocalView into Applications, replacing an older copy if present. Eject the disk image, then launch `/Applications/LocalView.app`.
...
```bash
shasum -a 256 LocalView_0.2.0-beta.6_aarch64.dmg
# Intel:
shasum -a 256 LocalView_0.2.0-beta.6_x64.dmg
```
...
```bash
hdiutil verify LocalView_0.2.0-beta.6_aarch64.dmg
